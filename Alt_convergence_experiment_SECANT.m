...
function Alt_convergence_experiment_SECANT()
...
x0_ref = 37.879;
% find ref root
target_root = fzero(@the_function,x0_ref);
abs_error_next=[];
abs_error_current=[];
...
f_record = my_recorder.generate_recorder_fun(@the_function);
...
x0_list = linspace(x0_ref-3,x0_ref+1,num_iter);
x1_list = linspace(x0_ref+1,x0_ref+5,num_iter);
...
for n = 1:num_iter
    ...
    [x_roots] = Alt_secant_solver_error(f_record,x0,x1,ftol,iter,max_iter,min_denom,dxtol);
